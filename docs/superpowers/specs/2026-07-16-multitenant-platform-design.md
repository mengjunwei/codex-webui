...
**M4 分布式(完整)**
- 所有节点均为 ingress + worker 一体（无角色分流）;`main.rs` 中每节点启动内网 RPC server + 心跳(带 rpc 地址)+ 空闲回收 + 快照
...
**部署**
- `docker-compose.yml`:PG + Redis + codex-webui(统一角色,多机可扩);`config.toml.example` 全量配置模板;README 同步 Rust 重写 + 多租户。
