...
- `event_bus.rs`:`EventBus` trait + `InMemoryEventBus`(单机) + **`RedisEventBus`(多机,已实现:`PUBLISH` + `SUBSCRIBE`→broadcast 带断线重连)**。3 单测验证 pub/sub / topic 隔离;`REDIS_URL` 配置启用,单机用本地 redis 验证
- 多机实现(`RedisRouter` 路由表按 `Router` trait 预留;`RedisEventBus` 已实现)按同套抽象。`Config.redis_url` + `AppState.mt_redis`(可选)集成,`REDIS_URL` 启用,单机用本地 redis 验证。接入 TeamCodexManager(notification 流 → RedisEventBus → 接入层 emit)留 M4 后期
...
**待办**:M5-B Prometheus 指标、M4 接入(notification 实时流 + failover + 接入/worker 分离)、M6 计费/安全审计

### ✅ M6-A 限流(2026-07-16)

- `rate_limit.rs`:`RedisRateLimiter`(Redis `INCR`+`EXPIRE` 固定窗口)
- 接入注册路由(`/api/mt/auth/register`):按 IP 每分钟 10 次,超限 429;Redis 未配置则跳过不禁用
- Redis 集成深化:事件总线(RedisEventBus)+ 限流(RedisRateLimiter)均落 Redis,多机一致

### ✅ M4/M5/M6 完整实现(2026-07-16,多机 SaaS 闭环)

本轮把设计文档剩余的分布式 / 高可用 / 计费能力全部落地(59 lib 单测全绿,0 回归)。

**M4 分布式(完整)**
- 节点角色 `NODE_ROLE`(ingress/worker/both):单二进制按角色分流;`main.rs` 中 worker/both 启动内网 RPC server + 心跳(带 rpc 地址)+ 空闲回收 + 快照;ingress/both 配路由器 + 转发客户端。
- 会话粘性 `RedisSticky`(thread→worker,TTL 续期)+ failover(粘性 worker 下线自动重路由,`mt_failover_total` 计数)。
- 路由接入:`RedisRouter`(一致性哈希)接入 `mt_create_thread`/`mt_start_turn`;本地 worker 短路,远程 worker 经 `WorkerRpcClient` 转发。
- worker 内网 RPC(`api/multitenant/internal_rpc.rs`):`/internal/{thread/start, turn/start, evict, approval/respond}`,内部 token 鉴权。
- CODEX_HOME 快照(`services/multitenant/snapshot.rs`):周期 backup_all(RPO),故障恢复 restore。
- 事件闭环 + 审批双保险(`services/multitenant/event_persist.rs`):codex notification + server_request → RedisEventBus → 接入层 emit;`event_persist` 订阅写 `pending_server_requests`/`turn_errors`(team_id);mt 审批列出/resolve(经路由回传 codex)。

**M3/M4 进程池调度(完整)**
- `TeamCodexManager` 重构:全局进程上限(LRU 回收腾位,满则 503 背压)、per-team 上限(配置就绪)、空闲 LRU 回收(15min)、每进程 semaphore(`ClientLease` 持有 permit,配合 write_tx 有界根治 OOM)。

**M5-B 可观测**
- Prometheus 关键指标散布:`codex_team_spawns_total` / `codex_team_evicts_total` / `codex_team_backpressure_total` / `codex_team_active_processes` / `mt_alive_workers` / `mt_failover_total` 等,`/metrics` 暴露。

**M6 计费 / 安全**
- 计费预留:`team_quotas` 表(m0006)+ `quota.rs`(per-team 每小时 turn 配额校验 + 用量计数,接入 `mt_start_turn`)。
- 限流覆盖:注册 + 登录(IP 每分钟 10 次,Redis 固定窗口)。
- 密钥轮转:`MASTER_KEY_PREVIOUS` 双密钥过渡(解密 fallback)+ 轮换串联(`set_team_api_key` 后 evict 旧进程 + 重写 auth.json)。

**硬伤修复**
- 业务表(`token_usage_snapshots` / `turn_diffs` / `turn_errors` / `pending_server_requests`)补 `team_id` 列 + 索引 + 回填(m0005);entity 同步;查询带 team 过滤(`mt_list_approvals`)。

**部署**
- `docker-compose.yml`:PG + Redis + codex-webui(both 角色,多机可扩);`.env.example` 全量多租户变量;README 同步 Rust 重写 + 多租户。

**数据层**:SeaORM 1.1(PG/MySQL),6 个迁移(initial / api_keys / audit / business / business_team_id / quota),rusqlite 已删。

### ✅ 骨架补全(2026-07-17,7 项简化版补完整)

第二轮把上一轮"简化版/骨架"的 7 项补完整(61 lib 单测全绿):

- **per-team 扩进程(多 slot)**:`TeamCodexManager` 每 team 维护 slot 列表,`client_for` 选最闲存活 slot;最闲 slot 并发达 `PROCESS_SCALE_THRESHOLD` 且未达 `MAX_PROCESSES_PER_TEAM` 时扩进程(`should_scale` 真正接入)。
- **team_routes failover 决策表**:`team_routes` 表(m0007)+ `RedisRouter.route` 先查覆盖表(worker 活则复用,死则 failover 落新 worker 并记 reason,防节点抖动回切)。
- **failover 恢复编排**:`TeamCodexManager` 持有 `snapshotter`,`spawn_slot` 前 CODEX_HOME 缺失则自动 `restore_team` → 新 worker 续接历史(thread/resume)。
- **token 用量持久化 + 月配额**:`event_persist` 解析 `thread/tokenUsage/updated` 写 `token_usage_snapshots`(team_id);`quota::incr_tokens` 累加月度 token(`used_tokens_month` + 跨月重置)。
- **mt 会话操作**:`/api/mt/threads/{id}/{token-usage,turn-diffs,turn-errors}`(读 PG)+ `archive`/`name`(更新 threads)+ 通用 `invoke`(经路由转发 codex:fork/rollback/resume)+ worker 内网 `/internal/thread/invoke`。
- **快照对象存储后端**:`StorageBackend` trait + `LocalBackend` + `S3Backend`(reqwest + 自实现 AWS Sig V4,不引入新 crate;配 `S3_*` 启用)。

> 验证边界:lib 编译 + 61 单测全绿。**端到端真实多机运行**(实际起 PG/Redis + 多 worker + 真实 codex + S3/MinIO + 真实流量)需在真实环境验证,本环境未跑。OAuth / SMTP 邮箱验证按原设计留二期。
...
- 现有 rusqlite 业务表 → SeaORM 迁移(已完成)

---

## 13. 架构演进:多副本 HA(2026-07-17,用户需求驱动)

按用户需求把多机模型从"per-team CODEX_HOME + 一致性哈希分片 + 快照恢复"改为
**多副本 active-passive HA**(全局 CODEX_HOME + session 同步 + 副本晋升):

- **全局 CODEX_HOME**(所有 team 共用;team 仅前端 UI 隔离——§3 的 per-team 目录隔离作废)。
- **per-team codex 进程**(各 team BYOK key,共享全局 CODEX_HOME)。
- **session 级 rollout 增量复制**:主每 turn 完成后扫 `CODEX_HOME/sessions/` 该 team 各 thread 的
  rollout,按 offset 取增量 POST 到副本 `/internal/replicate`;副本 append 到本地(offset 幂等)。
- **`session_replicas` 表**(m0008)替代 `team_routes`:team_id → primary_node + replica_node + lease。
- **探活**:`ClusterMembership` trait + `RedisCluster`(默认,心跳 TTL)+ `MemberlistCluster`
  (feature `memberlist-backend`;memberlist 0.8.5 gossip,transport/delegate 联调待部署期)。
- **副本晋升**:副本周期探测主失活(不在 alive + 租约过期)→ 升主 → 起 codex + 对所有活跃 thread
  调 `thread/resume`(persistExtendedHistory)续接。
- **移除**:`team_routes`/`ConsistentHash`/`RedisRouter`/`sticky`/S3 快照/per-team CODEX_HOME 目录。
- **保留**:`TeamCodexManager` 主体、`event_persist`(4 业务表)、`RedisEventBus`、限流、BYOK、配额、审批。

新增:`services/multitenant/{cluster,replication}.rs`、`m0008_session_replicas`、`session_replica` entity、
`internal_rpc /internal/replicate`、`main.rs` 主副本维护 task(续约/复制/晋升)。

> 验证边界:lib + bin 编译通过 + 54 单测全绿。memberlist 0.8.5 已加入依赖并编译通过(默认用 RedisCluster
> 探活,本环境可跑);全局 CODEX_HOME 多 team 进程 + 真实多副本 failover 需部署期实测。

## 多 team 部署 CODEX_HOME 建议

所有 team 共享全局 CODEX_HOME;codex 自管的 config.toml / history.sqlite 在多 team 进程并发下存在 sqlite locked / 数据串味风险。

多团队生产部署建议:
- **方案 A(推荐):** 每 team 一台独立 host,各自 CODEX_HOME。
- **方案 B(过渡):** per-team 子目录手工挂载,例如 `CODEX_HOME=/var/lib/codex/team-A`,在外部 LB 层按 team_id 路由到不同 host。
...
| 两副本同时发现主失活 | Redis SET NX 只有一个成功;失败者下次周期重试 | 0 |
