...
#
# 用法：
#   bash start-wsl.sh              # 启动全部（cc-switch + codex-webui）
#   bash start-wsl.sh stop         # 停止全部
#   bash start-wsl.sh restart      # 重启 codex-webui（cc-switch 不动）
#   bash start-wsl.sh restart-all  # 重启全部（cc-switch + codex-webui）
#   bash start-wsl.sh status       # 查看状态
#   bash start-wsl.sh switch xiaomi # 切换 provider（xiaomi/minimax）
#   bash start-wsl.sh logs         # tail -f codex-webui 日志
#
# 注意：此脚本需要在 master 用户下运行（su - master 后执行）
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
switch_provider() {
  local target="${1:-}"
  if [[ -z "$target" ]]; then
    err "用法：bash start-wsl.sh switch <provider-id>"
    err "可用 provider："
...
│  停止    : bash $DEPLOY_HOME/start-wsl.sh stop                           │
│  切换    : bash $DEPLOY_HOME/start-wsl.sh switch xiaomi                  │
└──────────────────────────────────────────────────────────────────────────┘
EOF
}
