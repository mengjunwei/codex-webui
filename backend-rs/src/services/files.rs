...
pub async fn compute_workspace_roots(db: &sea_orm::DatabaseConnection, dynamic_roots: &HashSet<String>, workspace_root: Option<&Path>) -> Vec<String> {
    ...
    // 多租户 workspace:workspace_root 下 users/ + teams/ 是各会话 cwd 的根目录,
...
pub async fn workspace_roots(state: &AppState) -> Vec<String> {
    let dyn_roots: HashSet<String> = state
        ...
        .unwrap_or_default();
    compute_workspace_roots(&state.db, &dyn_roots, Some(&state.workspace_root)).await
}

/// 判断规范化路径是否位于任一工作区根目录之下（含等于根本身）。
pub async fn is_path_in_workspace(db: &sea_orm::DatabaseConnection, dynamic_roots: &HashSet<String>, workspace_root: Option<&Path>, p: &Path) -> bool {
    let roots = compute_workspace_roots(db, dynamic_roots, workspace_root).await;
    let p_str = p.to_string_lossy().to_string();
    roots.iter().any(|r| is_within(p, Path::new(r)) || p_str == *r)
}
...
pub async fn resolve_terminal_cwd(
    ...
    if !is_path_in_workspace(db, dynamic_roots, Some(workspace_root), &canon).await {
        return Err(AppError::business(
            ErrorCode::FilesPathOutsideWorkspace,
            StatusCode::FORBIDDEN,
